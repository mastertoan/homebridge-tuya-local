...
class TuyaDiscovery extends EventEmitter {
    ...
    _handleResponse(msg, rinfo, callback) {
        ...
        try {
            ...
            if (msg.length >= 16 &&
                msg.readUInt32BE(0) === 0x000055aa &&
                msg.readUInt32BE(msg.length - 4) === 0x0000aa55) {

                this.log.debug(`Discovery - Raw message from ${rinfo.address}: ${msg.toString('hex')}`);

                // Try to parse the message
                try {
                    const data = msg.slice(20, msg.length - 8);
                    const decryptedMsg = this._decrypt(data);
                    const parsed = JSON.parse(decryptedMsg);

                    this.log.debug(`Discovery - Parsed message: ${JSON.stringify(parsed)}`);

                    if (parsed.ip && parsed.gwId) {
                        callback(null, {
                            id: parsed.gwId,
                            ip: parsed.ip,
                    ...
                    this.log.debug(`Discovery - Failed to parse message: ${parseError.message}`);
                }
            }
        } catch (error) {
            this.log.debug(`Discovery - Error handling response: ${error.message}`);
        }
