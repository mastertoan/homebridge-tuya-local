...
class TuyaDiscovery extends EventEmitter {
    ...
    _onDgramMessage(port, msg, info) {
        ...
        this.log.debug(`Discovery - Received UDP packet from ${info.address}:${port} length: ${len}`);
        ...
        try {
            ...
            if (result && result.gwId && result.ip) {
                this.log.info('Discovery - New device found:', result);
    ...
    _onDiscover(data) {
        ...
        this.log.info(`Discovery - New device found:
            ID: ${data.id}
            IP: ${data.ip}
            Version: ${data.version}
            Product Key: ${data.productKey || 'N/A'}
        `);
