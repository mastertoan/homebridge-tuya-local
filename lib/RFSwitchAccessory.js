...
class RFSwitchAccessory extends BaseAccessory {
    ...
    constructor(...props) {
        ...
        const deviceType = props[2].context.deviceType || 4;  // props[2] là device
        this.numChannels = deviceType === 7 ? 7 : 
                        deviceType === 8 ? 8 : 4;

        // Khởi tạo dpSwitches trước
        this.dpSwitches = {};
        ...
        // Gọi super sau khi đã khởi tạo các thuộc tính cần thiết
        super(...props);
    ...
    _registerPlatformAccessory() {
        ...
        // Kiểm tra xem dpSwitches đã được khởi tạo chưa
        if (!this.dpSwitches) {
            this.log.error('dpSwitches not initialized');
            return;
        }
        ...
        for (let i = 1; i <= this.numChannels; i++) {
            try {
                ...
                this.log.error(`Error adding service for switch ${i}:`, err);
            }
        }
