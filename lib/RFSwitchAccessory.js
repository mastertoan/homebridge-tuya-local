...
class RFSwitchAccessory extends BaseAccessory {
    ...
    _registerPlatformAccessory() {
        ...
        if(!this.dpSwitches || this.dpSwitches.length === 0) {
            for (let i = 1; i <= 4; i++) {
                this.dpSwitches[i] = {
                    code: `switch_${i}`,
                    name: `Switch ${i}`
        ...
        this.log.info('_registerPlatformAccessory:', this.numChannels);

        
        // Tạo service cho mỗi kênh
        for (let i = 1; i <= this.numChannels; i++) {
            const existingService = this.accessory.getService(this.dpSwitches[i].name);
            if (!existingService) {
                this.log.debug(`Adding switch service ${i}`);
                this.accessory.addService(Service.Switch, this.dpSwitches[i].name, `switch${i}`);
            } else {
                this.log.debug(`Service ${this.dpSwitches[i].name} already exists`);
            }
        }
        ...
        for (let i = 1; i <= this.numChannels; i++) {
            ...
            service.getCharacteristic(Characteristic.On)
                ...
                .on('set', (value, callback) => {
                    ...
                    // Gửi command qua API
                    this.device.sendCommands(commands)
                        .then(() => {
