...
class RFSwitchAccessory extends BaseAccessory {
    ...
    _registerPlatformAccessory() {
        ...
        try {
            ...
            for (let i = 1; i <= this.numChannels; i++) {
                ...
                if (!existingService) {
                    this.log.info(`Adding switch service ${i}: ${switchName}`);
                    this.accessory.addService(Service.Switch, switchName, `switch${i}`);
                } else {
                    this.log.info(`Service ${switchName} already exists`);
                }
            }
    ...
    _registerCharacteristics(dps) {
        ...
        for (let i = 1; i <= this.numChannels; i++) {
            ...
            this.log.info(`Setting up ${this.dpSwitches[i].name} with code:`, dpCode);

            service.getCharacteristic(Characteristic.On)
                .on('get', callback => {
                    this.log.info(`-> Get ${dpCode}`);
                    this.getState(dpCode, (err, dp) => {
                        if (err) {
                            this.log.error(`Error getting state for ${dpCode}:`, err);
                            callback(err);
                        } else {
                            this.log.info(`Raw state for ${dpCode}:`, dp);
                            const value = this._getSwitch(dp);
                            this.log.info(`<- ${dpCode} is ${value}`);
                            callback(null, value);
                        }
                    });
                })
                .on('set', (value, callback) => {
                    this.log.info(`-> Set ${dpCode} to ${value}`);
                    ...
                    this.sendCommands(commands)
                        .then(() => {
                            this.log.info(`<- ${dpCode} command sent successfully`);
                            // Cập nhật trạng thái ngay lập tức
                            this.device.state[dpCode] = value;
                            callback();
                        })
                        .catch(err => {
        ...
        this.device.on('change', (changes, state) => {
            ...
            for (let i = 1; i <= this.numChannels; i++) {
                ...
                if (changes.hasOwnProperty(dpCode)) {
                    ...
                    this.log.info(`Raw change for ${dpCode}:`, changes[dpCode]);
                    const newValue = this._getSwitch(changes[dpCode]);
                    this.log.info(`Updating ${dpCode} to ${newValue}`);
    ...
    _getSwitch(value) {
        if (value === undefined) {
            this.log.info('Switch value is undefined, defaulting to false');
            return false;
        }
        // Chuyển đổi giá trị thành boolean
        const boolValue = Boolean(value);
        this.log.info(`Converting switch value: ${value} -> ${boolValue}`);
        return boolValue;
    }
    ...
    sendCommands(commands) {
        return new Promise((resolve, reject) => {
            ...
            this.log.info('Sending commands:', JSON.stringify(commands));

            // Chuyển đổi commands thành format dps
            const dps = {};
            commands.forEach(cmd => {
                dps[cmd.code] = cmd.value;
            });

            // Tạo message theo protocol 3.4
            const message = {
                gwId: this.device.context.gwId,
                devId: this.device.context.devId,
                uid: this.device.context.uid,
            ...
            // Gửi lệnh qua device
            this.device._send({
                cmd: 7, // Command code for control
                data: message
            }, null, (err, data) => {
                if (err) {
                    ...
                    this.log.info('Commands sent successfully:', data);
                    resolve(data);
                }
            });
        });
