...
class TuyaAccessory extends EventEmitter {
    ...
    _connect() {
        ...
        this.log.info(`${this.context.name} - Starting connection:
            IP: ${this.context.ip}
            Port: ${this.context.port || 6668}
            Version: ${this.context.version}
            Protocol: ${this.context.protocol || null}
            Connected: ${this.connected}
            Socket Status: ${this._socket ? (this._socket.destroyed ? 'Destroyed' : 'Active') : 'Not Created'}
        `);
        ...
        (this._socket.reconnect = () => {
            ...
            }, (this.context.connectTimeout || 60) * 1000);
            ...
            this._socket.connect({
                ...
                timeout: (this.context.connectTimeout || 60) * 1000
            });
        })();
        ...
        this._socket.on('connect', () => {
            ...
            this.log.info(`${this.context.name} - Socket connected, preparing handshake data`);
        
            if (this.context.version === '3.4') {
                try {
                    // Tạo dữ liệu handshake theo đúng format cho version 3.4
                    const localKey = crypto.randomBytes(16);
                    const cipher = crypto.createCipheriv('aes-128-ecb', this.context.key, '');
                    let encrypted = cipher.update(localKey);
                    encrypted = Buffer.concat([encrypted, cipher.final()]);

                    this.log.info(`${this.context.name} - Handshake data prepared:
                        Local Key Length: ${localKey.length}
                        Encrypted Length: ${encrypted.length}
                        Key Used: ${this.context.key.slice(0, 8)}... (truncated)
                    `);

                    // Gửi handshake với format mới
                    const payload = {
                        data: encrypted,
                        encrypted: true,
                        cmd: 3,
                    ...
                    this.log.info(`${this.context.name} - Sending handshake payload:
                        Command: ${payload.cmd}
                        Protocol: ${payload.protocol}
                        Timestamp: ${payload.t}
                        Encrypted Length: ${payload.data.length}
                    `);

                    this._send(payload);
                } catch (err) {
                    this.log.info(`${this.context.name} - Handshake preparation failed: ${err.message}`);
        ...
        this._socket.on('ready', () => {
            ...
            this.log.debug(`${this.context.name} - Socket connected, attempting handshake`);
        ...
        this._socket.on('data', msg => {
            ...
            this.log.info(`${this.context.name} - Received data: ${msg.length} bytes`);
        ...
        this._socket.on('error', err => {
            this.connected = false;
            this.log.info(`${this.context.name} - Socket error details:
                Error: ${err.message}
                Code: ${err.code}
                Local address: ${this._socket.localAddress}
                Local port: ${this._socket.localPort}
                Remote address: ${this._socket.remoteAddress}
                Remote port: ${this._socket.remotePort}
                Socket readyState: ${this._socket.readyState}
            `);

            if (err.code === 'ECONNRESET' || err.code === 'EPIPE' || err.code === 'ERR_PING_TIMED_OUT') {
                if (this._connectionAttempts < (this.context.maxRetries || 10)) {
                    this.log.debug(`Attempting reconnect for ${this.context.name}`);
        ...
        this._socket.on('close', err => {
            ...
            this.log.info(`${this.context.name} - Socket close details:
                Had error: ${this._socket.hadError}
                Destroyed: ${this._socket.destroyed}
                Connected before: ${this.connected}
    ...
    _msgHandler_3_3(task, callback) {
        ...
        this.log.debug(`${this.context.name} - Processing message:
            Command: ${task.cmd}
            Length: ${task.msg ? task.msg.length : 0}
        `);
    ...
    _send_3_4(o) {
        ...
        this.log.info(`${this.context.name} - Sending 3.4 protocol message:
            Command: ${cmd}
