...
class TuyaAccessory extends EventEmitter {
    constructor(props) {
        ...
        // Khởi tạo this.log trước
        this.log = props.log;

        // Sau đó mới log
        this.log.info('Initializing TuyaAccessory:', {
            id: props.id,
            name: props.name,
            type: props.type,
            ip: props.ip,
        ...
        if (!(props.id && props.key && props.ip) && !props.fake) {
            return this.log.info('Insufficient details to initialize:', props);
        }
        ...
        this.connectTimeout = 30000;  // Tăng timeout lên 30s
        this.pingPongPeriod = 15000; // Tăng thời gian ping
        this.maxReconnects = 5;      // Tăng số lần thử kết nối lại

        // Force connect nếu là direct connect
        if (props.connect === true) {
            this._connect();
        }
    }

    _connect() {
        this.log.info(`Direct connecting to ${this.context.name} (${this.context.ip}:${this.context.port})`);
        ...
        (this._socket.reconnect = () => {
            ...
            // Tăng buffer size bằng cách sử dụng các option có sẵn
            this._socket.setTimeout(60000); // 60s timeout
            ...
            this._socket.connect({
                ...
                // Thêm các tùy chọn TCP để tối ưu kết nối
                keepAlive: true,
                noDelay: true,
                timeout: 60000
            });
        ...
        this._socket._ping = () => {
            this.log.debug(`Sending ping to ${this.context.name}`);
            if (this._socket._pinger) {
                this.log.debug(`Setting ping timeout for ${this.context.name}`);
                clearTimeout(this._socket._pinger);
                this._socket._pinger = setTimeout(() => {
                    this.log.error(`Ping timeout for ${this.context.name}`);
        ...
        this._socket.on('connect', () => {
            this.log.info(`Connected to ${this.context.name}`);
        ...
        this._socket.on('error', err => {
            this.log.error(`Connection error for ${this.context.name}:`, {
                error: err.message,
                code: err.code
            });
            
            // Retry nhanh hơn với direct connect
            if (this.context.connect === 'direct') {
                setTimeout(() => {
                    this._connect();
                }, 5000);
            }
        ...
        this._socket.on('close', err => {
            this.log.debug(`Connection closed for ${this.context.name}`, err ? `with error: ${err}` : '');
            this.connected = false;
            this.session_key = null;
    ...
    _msgHandler_3_1(task, callback) {
        ...
        this.log.debug(`Received message from ${this.context.name}:`, {
            command: cmd,
            data: data,
            raw: task.msg.toString('hex')
        });
    ...
    update(o) {
        this.log.debug(`Updating ${this.context.name}:`, o);
        ...
        let result = false;
        if (hasDataPoint) {
            this.log.info(`Sending update to ${this.context.name}:`, dps);
    ...
    _change(data) {
        if (!isNonEmptyPlainObject(data)) {
            this.log.debug(`Invalid data received for ${this.context.name}:`, data);
            return;
        }
        ...
        Object.keys(data).forEach(key => {
            if (data[key] !== this.state[key]) {
                ...
                this.log.debug(`State change for ${this.context.name} [${key}]:`, {
                    from: this.state[key],
                    to: data[key]
                });
            }
    ...
    _send(o) {
        if (!this.connected) {
            this.log.debug(`Cannot send to ${this.context.name} - not connected`);
            return false;
        }
        
        this.log.debug(`Sending to ${this.context.name}:`, {
            command: o.cmd,
            data: o.data
        });
